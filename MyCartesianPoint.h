...
class CartesianPoint
{
  ...
  /* Accessors: Used to query the state of the object */    
	...
	// Create a function to calculate the total distance
	static double TotalDistance(vector<CartesianPoint> myPoints);
...
/** TotalDistance Function to calculate the total distance
  * for the Lab 6 - Vectors assignment
  */
static double CartesianPoint::TotalDistance(vector<CartesianPoint> myPoints)
{
  	double totalDistance = 0.0;
  	for (int i=0; i<(myPoints.size()-1); i++)
    {
        totalDistance += myPoints[i].GetDistanceTo(myPoints[i+1]);
    }
